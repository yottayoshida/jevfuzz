...
async function sourceFreeze(manifestText: string) {
  const files = ['scripts/benchmark-v09.ts', 'scripts/benchmark-v09-supplemental.ts', 'docs/benchmark-v09.md', 'package-lock.json'];
...
async function main() {
  ...
  const reportAnalysis = analysis(rows, manifest.full.logicalRequests);
