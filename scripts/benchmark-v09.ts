...
import { v4SimulatorActive } from './benchmark-v09-simulator.ts';

type Family = { id: string; expect: 'detect' | 'hold' | 'inconclusive' };
type Manifest = { version: number; kind: string; mode?: string; seedSchedule?: string; families: Family[]; strategies: CampaignConfig['search']['strategy'][]; full: { seeds?: number; seedStart?: number; seedCount?: number; logicalRequests: number; httpAttempts: number }; fixedStatNull: { campaigns: number; familySlots: number; pairs: number; alpha: number } };
type Trial = { version: number; sourceHash: string; firstConfirmationObservationOrdinals: number[]; seed: number; family: string; expected: Family['expect']; strategy: string; status: string; exitCode: number; findings: number; actionableFindings: number; discoveryEvent: boolean; firstConfirmedCandidateOrdinal: number | null; firstFindingLogicalCall: number; firstFindingCensored: boolean; logicalCalls: number; httpAttempts: number; phaseCosts: Record<string, number>; invalidCandidates: number; controls: number; confirmationSamples: number; signatureCount: number; stableSignatureCount: number; observedModels: string[]; wireSequenceHash: string; elapsedMs: number };
const ROOT = resolve(import.meta.dirname, '..');
const MANIFEST = join(ROOT, 'fixtures/benchmarks/manifest.json');
const V4_MANIFEST = join(ROOT, 'fixtures/benchmarks/manifest-v4.json');
const DEVELOPMENT_MANIFEST = join(ROOT, 'fixtures/benchmarks/manifest-development.json');
const DEFAULT_OUT = join(ROOT, 'fixtures/benchmarks/raw-v09-v3');
const V4_DEFAULT_OUT = join(ROOT, 'fixtures/benchmarks/raw-v09-v4');
const V4_FAMILIES = ['order', 'metadata', 'interaction2', 'interaction3', 'rareconfident', 'noise', 'drift', 'choice-invariant', 'noul-invariant', 'score-invariant', 'policy-confidence', 'policy-choice'];
const V4_EXPECTATIONS = ['detect', 'detect', 'detect', 'detect', 'detect', 'hold', 'inconclusive', 'hold', 'hold', 'hold', 'detect', 'detect'];
const STRATEGIES = ['enumerator', 'uniform', 'novelty', 'boundary', 'feedback'];
...
function active(family: string, request: JevRequest, version: number) {
  const criteria = Object.values(request.questions).some(q => q.type === 'choice' && Object.keys(q.criteria)[0] === 'general');
  if (version === 4) return v4SimulatorActive(family, request);
...
function simulated(family: Family, request: JevRequest, index: number, version = 3): JevResponse {
  const changed = active(family.id, request, version), model = family.id === 'drift' && index % 5 === 4 ? 'benchmark-drift-b' : 'benchmark-a';
...
async function sourceFreeze(manifestText: string, version: number) {
  ...
  if (version === 4) files.push('scripts/benchmark-v09-v4.ts', 'scripts/benchmark-v09-simulator.ts', 'fixtures/benchmarks/manifest-v4.json', 'fixtures/benchmarks/manifest-development.json', 'docs/benchmark-feedback-v4.md');
  ...
  const frozen = { version, manifestHash: sha(manifestText), node: process.version, sources };
...
function validateRows(rows: Trial[], expected: Set<string>, sourceHash: string, version: number, complete = false) {
  const seen = new Set<string>();
  for (const row of rows) {
    const key = tuple(row);
    if (row.version !== version || row.sourceHash !== sourceHash) throw new Error(`unfrozen or incompatible raw trial: ${key}; historical endpoint data is diagnostic only`);
...
function validateV4Manifest(manifest: Manifest, diagnostic: boolean) {
  const expectedKind = diagnostic ? 'jevfuzz-v09-benchmark-manifest-v4-development' : 'jevfuzz-v09-benchmark-manifest-v4';
  const expectedStart = diagnostic ? 0 : 10000;
  const expectedCount = diagnostic ? 10 : 100;
  if (manifest.version !== 4 || manifest.kind !== expectedKind || manifest.seedSchedule !== 'mulberry32-v1' || manifest.full.seedStart !== expectedStart || manifest.full.seedCount !== expectedCount || manifest.full.logicalRequests !== 1000 || manifest.full.httpAttempts !== 5000) throw new Error('v4 manifest does not match the frozen protocol');
  if (manifest.families.length !== 12 || manifest.families.map(family => family.id).join(',') !== V4_FAMILIES.join(',') || manifest.families.map(family => family.expect).join(',') !== V4_EXPECTATIONS.join(',') || manifest.strategies.join(',') !== STRATEGIES.join(',')) throw new Error('v4 manifest must retain the registered families, expectations, and strategies');
  if (manifest.fixedStatNull.campaigns !== 1000 || manifest.fixedStatNull.familySlots !== 10 || manifest.fixedStatNull.pairs !== 64 || manifest.fixedStatNull.alpha !== .05) throw new Error('v4 manifest must retain fixed-stat null calibration');
}
async function main() {
  const flags = parseArgs({ options: { quick: { type: 'boolean' }, out: { type: 'string' }, manifest: { type: 'string' }, 'supplemental-only': { type: 'boolean' }, 'resume-trials': { type: 'boolean' }, 'trials-only': { type: 'boolean' }, 'shard-index': { type: 'string' }, 'shard-count': { type: 'string' }, 'merge-shards': { type: 'string' } } }).values;
  const manifestPath = resolve(flags.manifest ?? MANIFEST);
  if (![MANIFEST, V4_MANIFEST, DEVELOPMENT_MANIFEST].includes(manifestPath)) throw new Error('only registered benchmark manifests may be run');
  if (manifestPath === V4_MANIFEST && process.env.JEVFUZZ_BENCHMARK_PROTOCOL !== 'v4') throw new Error('run the canonical v4 manifest through scripts/benchmark-v09-v4.ts');
  const manifestText = await readFile(manifestPath, 'utf8'), manifest = JSON.parse(manifestText) as Manifest;
  const diagnostic = manifestPath === DEVELOPMENT_MANIFEST;
  if (manifestPath === V4_MANIFEST || manifestPath === DEVELOPMENT_MANIFEST) validateV4Manifest(manifest, diagnostic);
  const protocolVersion = manifest.version === 4 ? 4 : 3;
  if (manifestPath === V4_MANIFEST && flags.quick) throw new Error('canonical v4 does not permit quick mode; use the development manifest for diagnostics');
  if (diagnostic && flags.quick) throw new Error('the development manifest always runs all five strategies');
  const out = resolve(flags.out ?? (manifest.version === 4 ? V4_DEFAULT_OUT : DEFAULT_OUT)), trialPath = join(out, 'trials.jsonl');
  const seedCount = manifest.full.seedCount ?? manifest.full.seeds;
  if (!seedCount) throw new Error('manifest must declare a positive seed count');
  const seeds = flags.quick ? 1 : seedCount, seedStart = flags.quick ? (manifest.full.seedStart ?? 0) : (manifest.full.seedStart ?? 0), strategies = flags.quick ? ['uniform'] as const : manifest.strategies;
  ...
  const specs = Array.from({ length: seeds }, (_, offset) => seedStart + offset).flatMap(seed => manifest.families.flatMap(family => strategies.map(strategy => ({ seed, family, strategy })))).filter((_, i) => i % shardCount === shardIndex);
  const expected = new Set(specs.map(spec => tuple({ seed: spec.seed, family: spec.family.id, strategy: spec.strategy })));
  const frozen = await sourceFreeze(manifestText, protocolVersion);
  ...
  if (continuing) {
    ...
    rows.push(...await readRows(trialPath)); validateRows(rows, expected, frozen.sourceHash, protocolVersion, Boolean(flags['supplemental-only']));
  } else {
  ...
  if (flags['merge-shards']) {
    ...
    validateRows(rows, expected, frozen.sourceHash, protocolVersion, true);
    ...
    for (const { seed, family, strategy } of specs) {
      ...
      const provider = new FakeProvider((request, index) => {
        ...
        const response = simulated(family, request, index, protocolVersion);
  ...
  validateDerivedRows(rows, manifest, { version: protocolVersion, runtime: { seedStart, seeds, strategies: strategies.length, trialRows: expected.size } }, frozen.sourceHash);
  ...
  for (let seed = 0; seed < (flags.quick ? 2 : 50); seed++) {
    ...
    for (const concurrency of [1, 4, 8]) {
      ...
      const report = await campaign(trial, new FakeProvider((request, index) => { sequence[index] = wireHash(JSON.stringify(request)); return simulated(manifest.families[0]!, request, index, protocolVersion); }), { persist: false });
  ...
  const expectations = { tupleCoverage: rows.length === expected.size, trialRuntimeErrorsAbsent: rows.every(row => row.status === 'complete'), invariantAndNoiseFalseFindingsAbsent: rows.filter(row => row.expected === 'hold').every(row => !row.discoveryEvent), driftInconclusive: rows.filter(row => row.expected === 'inconclusive').every(row => row.exitCode === 3), knownMinimumShapes: shrinkSuite.passed && shrinkSuite.trials.length === shrinkCount, concurrencyStable: concurrencyRows.every(row => row.stable), nullSlotsCovered: nullSuite.originalSlots === 5 && nullSuite.shrinkSlots === 5 && nullSuite.adaptiveFinalConfirmations > 0 && nullSuite.trials.every(row => row.original.length === 5 && row.shrink.length === 5), nullFwerUpperAtMost007: flags.quick ? null : nullUpper <= .07, guardScenarios: supplemental.every(row => !row.falseFail), sourcesUnchanged: JSON.stringify(await sourceFreeze(manifestText, protocolVersion)) === JSON.stringify(frozen) };
  ...
  const canonicalV4Full = protocolVersion === 4 && !diagnostic && !flags.quick && !flags['trials-only'] && rows.length === 6000;
  if (flags.quick || !passed || diagnostic || (protocolVersion === 4 && !canonicalV4Full)) { reportAnalysis.promotion.feedbackPromotion = 'not promoted'; reportAnalysis.promotion.reason = diagnostic ? 'Diagnostic development runs structurally cannot establish promotion criteria.' : flags.quick ? 'Quick smoke cannot establish the registered promotion criteria.' : protocolVersion === 4 && !canonicalV4Full ? 'Only a complete canonical v4 run can establish promotion criteria.' : 'Protocol acceptance failed; performance data cannot promote feedback.'; }
  const summary = { version: protocolVersion, kind: protocolVersion === 4 ? 'jevfuzz-v09-benchmark-summary-v4' : 'jevfuzz-v09-benchmark-summary', status, command: process.argv.join(' '), runtime: { mode: diagnostic ? 'offline-fake-provider-diagnostic' : 'offline-fake-provider', node: process.version, seeds, seedStart, families: manifest.families.length, strategies: strategies.length, trialRows: rows.length, logicalBudgetPerTrial: manifest.full.logicalRequests, httpBudgetPerTrial: manifest.full.httpAttempts }, sourceHashes: frozen, raw: { directory: out, trials: 'trials.jsonl', retainedInRepository: out.startsWith(join(ROOT, 'fixtures')) }, acceptance: { passed, expectations }, aggregate: { trialErrors: rows.filter(row => row.status !== 'complete').length, fixedStatNull: { campaigns: nullSuite.trials.length, originalSlots: nullSuite.originalSlots, shrinkSlots: nullSuite.shrinkSlots, pairs: profile.pairs, falseFails: nullSuite.falseFails, adaptiveFinalConfirmations: nullSuite.adaptiveFinalConfirmations, adaptiveCampaignCoverage: nullSuite.adaptiveFinalCoverage, clopperPearsonUpper95: nullUpper }, supplementalNullScenarios: supplemental, shrink: { required: shrinkCount, executed: shrinkSuite.trials.length, knownMinimumShapesPassed: shrinkSuite.passed }, concurrency: { seeds: concurrencyRows.length, allStable: concurrencyRows.every(row => row.stable) } }, analysis: reportAnalysis, decision: reportAnalysis.promotion };
  await writeFile(join(out, 'summary.json'), JSON.stringify(summary, null, 2));
  if (!flags.quick && protocolVersion === 3 && out === DEFAULT_OUT) await writeFile(join(ROOT, 'fixtures/benchmarks/results-v09.json'), JSON.stringify(summary, null, 2));
